/* =====================================================
   BIG DONER — menu data + cart + WhatsApp redirect
   ===================================================== */

const PHONE = "77750804050"; // international format, no "+"

/* ---------- 0. CONFIG ---------- */
const DELIVERY_ENABLED   = true;
const DELIVERY_THRESHOLD = 5000; // ₸ — заказы от этой суммы получают бесплатную доставку
const DELIVERY_FEE       = 700;  // ₸ — взимается, если сумма заказа ниже порога
const WORK_HOURS         = "10:00 – 02:00";

/* ---------- 1. MENU ---------- */
/* MENU и ITEMS теперь определены в menu-data.js (общий файл с панелью оператора) */

/* ---------- 2. STATE ---------- */
const state = { unavailable: new Set(), fulfillment: "delivery" };   // subTotal, delivery, grand, unavailable — популируются ниже
let cart = {};      // { id: qty }
...
function renderMenu(){
  ...
  wrap.innerHTML = MENU.map((g)=>`
    <section class="cat-section" id="cat-${g.id}" style="grid-column:1/-1">
      <h2 class="cat-heading"><span class="rule"></span><span class="cat-heading-text">${g.name}</span><span class="rule"></span></h2>
    </section>
    ${g.items.map((item)=>{
...
function updateCart(){
  ...
  /* delivery fee (только если выбрана доставка — при самовывозе комиссия не начисляется) */
  ...
  if(!isDelivery){
    fee = 0;
    hint = "";
    $("#deliveryLabel").textContent = "Получение";
    $("#deliveryFee").textContent = "Самовывоз";
  } else if(!DELIVERY_ENABLED){
    fee = 0;
    hint = total ? "стоимость уточнит оператор" : "";
    $("#deliveryLabel").textContent = "Доставка";
    $("#deliveryFee").textContent = "уточняется";
  } else if(total === 0){
    fee = 0; hint = "";
    $("#deliveryLabel").textContent = "Доставка";
    $("#deliveryFee").textContent = "бесплатно";
  } else if(total < DELIVERY_THRESHOLD){
    fee = DELIVERY_FEE;
    const need = DELIVERY_THRESHOLD - total;
    hint = `+${need.toLocaleString("ru-RU")} ₸ до бесплатной доставки`;
    $("#deliveryLabel").textContent = "Доставка";
    $("#deliveryFee").textContent = `+${fee.toLocaleString("ru-RU")} ₸`;
  } else {
    fee = 0;
    hint = "бесплатно 🎉";
    hintClass = "success";
...
/* ---------- 4d. ДОСТАВКА / САМОВЫВОЗ ---------- */
function setFulfillment(type){
  state.fulfillment = type;
  $$(".fulfill-btn").forEach(b => b.classList.toggle("active", b.dataset.type === type));
...
function buildMessage(){
  ...
  if(isDelivery){
    ...
    gate = $("#custGate").value.trim();
    if(!name || !addr){ alert("Пожалуйста, заполните имя и адрес доставки."); return null; }
  } else {
    if(!name){ alert("Пожалуйста, укажите имя."); return null; }
  }
  ...
  const feeLine = !isDelivery
    ...
    : DELIVERY_ENABLED
      ? (fee ? `• Доставка: +${fee.toLocaleString("ru-RU")} ₸` : `• Доставка: бесплатно`)
  ...
  return [
    ...
    "",
    `  Имя: ${name}`,
    `  Способ получения: ${isDelivery ? "Доставка" : "Самовывоз"}`,
    isDelivery ? `  Адрес: ${addr}` : null,
    isDelivery && gate ? `  Домофон/этаж: ${gate}` : null,
    `  Оплата: ${pay}`,
    "",
    " Заказ: ",
    ...lines,
...
function init(){
  ...
  $$(".fulfill-btn").forEach(btn=>{
    btn.addEventListener("click", ()=> setFulfillment(btn.dataset.type));
  });
